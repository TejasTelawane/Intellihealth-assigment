...
//                        .skipMemoryCache(true)
    ...
    public static void showImageDialog(Activity activity, File imageFile, String url, int flag) {
        ...
        ImageView img_download = (ImageView) dialog.findViewById(R.id.img_download);
...
//                    .diskCacheStrategy(DiskCacheStrategy.NONE)
//                    .skipMemoryCache(true)
                    .error(activity.getResources().getDrawable(R.drawable.ic_app_logo))
                    .into(imgView);
        }
        img_download.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                PermissionUtils.AsKPermissionToDownloadImage(activity, new PermissionUtils.OnPermissionSuccessListener() {
                    @Override
                    public void onPermissionGranted() {
                        if (flag == 2){
                            CommonUtils.DownloadFile(activity,url,"Image_"+System.currentTimeMillis());
                        }else{
                            CommonUtils.showToast(activity, "Image url not found");
                        }
